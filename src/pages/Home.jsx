...
import TruncatedDescription from '../components/TruncatedDescription.jsx';
    ...
    <div className='bg-primary'>     
      <main>
        <div className="bg-white max-w-[1200px] mx-auto" >
          <Navbar />
          <img className='max-w-[1000px] h-auto mx-auto ' src="src/assets/images/curtain.jpg" alt="HeroImage" />
        </div>
        <div className="max-w-[1200px] mx-auto bg-white" id='wrapper'>
        <section className='max-w-[1000px] mx-auto bg-white p-6'>
            <h3 className='text-orange text-3xl font-bold font-titillium-web mb-12'>
              Sidste nyt...
            </h3>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4 text-darkGray'>
              {posterData.map((poster) => (
                <div className='flex-auto w-full flex flex-col md:flex-row lg:flex-row xl:flex-row items-center' key={poster.id}>
                  <img className='h-76 md:w-1/2 lg:w-1/2 xl:w-1/2 object-cover cursor-pointer' src={poster.image} alt={poster.name} onClick={() => openModal(poster)} />
                  <div className='ml-4 md:w-1/2 lg:w-1/2 xl:w-1/2'>
